...
class Network_cifar3_3(nn.Module):
    def __init__(self, num_classes=10):
        super(Network_cifar3_3, self).__init__()
        channels = {'prep': 64, 'layer1': 128, 'layer2': 256, 'layer3': 512}
        # prep
        self.conv1 = self.conv3x3(3, channels['prep'])
        self.bn1 = nn.BatchNorm2d(channels['prep'])
        ...
        self.layer1_features.append(
            BasicBlock(channels['layer1'], channels['layer1'], stride=1, choice=0))
        ...
        self.layer3_features.append(
            BasicBlock(channels['layer3'], channels['layer3'], stride=1, choice=2)) #
...
if __name__ == '__main__':
    ...
    # 两个layer中的内部conv相同
    # (3,3), (3,3):6.62  M 0.38 GMac
    # (3,5), (3,5):11.08 M 0.51 GMac  -> # (5,3), (5,3):11.08 M 0.51 GMac
    # (3,7), (3,7):17.76 M 0.72 GMac

    # (5,5), (5,5):15.53 M 0.65 GMac
    # (5,7), (5,7):22.22 M 0.85 GMac
    # (7,7), (7,7):28.9  M 1.05 GMac

    # 每个layer中的内部conv不同
    # (3,3), (3,7):17.1 M  0.55 GMac

    inputs = torch.randn(3, 32, 32).unsqueeze(0)
    model = Network_cifar3_3(num_classes=100)
